...
{
#if false
    [Command("updatemerged", "Updates a merged install tree")]
    public class UpdateMergedCommand : Command, ILogger
    {
        private readonly List<string> _sources = new List<string>();
        private readonly List<string> _ids = new List<string>();
        private bool _overwriteAll, _ignoreAll;

        [Option("The source to search for updates")]
        public ICollection<string> Source
        {
            get { return _sources; }
        }

        [Option("ID of the package to update")]
        public ICollection<string> Id
        {
            get { return _ids; }
        }

        [Option("Location where the packages are")]
        public string RepositoryPath { get; set; }

        [Option("Only search for compatible versions")]
        public bool Safe { get; set; }

        [Option("Be verbose in output")]
        public bool Verbose { get; set; }

        [Option("Include prerelease versions")]
        public bool Prerelease { get; set; }

        [Option("What to do in case of file conflicts")]
        ...
        public override void ExecuteCommand()
        {
            ...
            {
                throw new CommandLineException("Invalid File"); // (NuGetResources.InvalidFile);
            }
            ...
            if (ProjectHelper.SupportedProjectExtensions.Contains(Path.GetExtension(inputFile) ?? string.Empty))
            {
                ...
                {
                    throw new CommandLineException("Unable to find project: {0}", /*NuGetResources.UnableToFindProject, */inputFile);
                }
            ...
            if (!FileSystem.FileExists(inputFile))
            {
                throw new CommandLineException("Unable to find solution: {0}", /*NuGetResources.UnableToFindSolution, */inputFile);
            }
        ...
        private void UpdatePackages(IMSBuildProjectSystem project, string repositoryPath = null, IPackageRepository sourceRepository = null)
        {
            ...
            var pathResolver = new MergedPackagePathResolver(sharedRepositoryFileSystem);
            ...
            var localRepository = new PackageReferenceRepository(sharedRepositoryFileSystem, "", sharedPackageRepository);
        ...
        private string GetRepositoryPathFromSolution(string solutionDir)
        {
            ...
            {
                packagesDir = Path.Combine(solutionDir, "packages"); //CommandLineConstants.PackagesDirectoryName);
            }
